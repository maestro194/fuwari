---
title: Kỹ thuật sinh test
published: 2025-12-21
tags: [Guides, Contest, Experience]
category: Guides
draft: false
---
...
Trong C++ có một số thư viện / hàm giúp chúng ta tạo ra các số random như ```rand()``` hay ```mt19937```. Mình khuyên các bạn sử dụng thư viện ```mt19937``` để có thể sinh những số lớn.

Có $2$ cách chính để sử dụng ```mt19937``` trong sinh test:
...
Để test code, ta chỉ cần chỉnh limit các biến trong test lên mức tối đa mà đề bài cho, cũng như bỏ code trâu (vì nếu để lại sẽ chạy rất lâu). Ngoài ra, để kiểm tra xem code chạy chính xác mất bao lâu trong một test, ta có thể sử dụng hàm ```clock()``` như sau:
...
Lưu ý: Hàm ```clock()``` trả về số tick kể từ lúc code bắt đầu chạy đến thời điểm được gọi. Do đó, để biết được số giây đã trôi qua, ta chia ```clock()``` cho ```CLOCKS_PER_SEC``` -- số tick trên giây.
...
## Template
Các bạn nhìn code generate ở trên thì có thể thấy khá dài, tuy nhiên template nếu đã nắm được thì có thể code chỉ trong  phút, ngoài ra còn có thể áp dụng được cho tất cả các bài trong kì thi.
...
Chẳng hạn như để sinh ra một cây (đồ thị với  đỉnh và  cạnh), ta có một số cách sinh sau đây:
...
- Cách sinh đơn giản nhất:
    ...
    Đây là cách nhanh và an toàn nhất để sinh ra cây, tuy nhiên hai nhuợc điểm của cách sinh test này là cây sẽ có độ cao thấp (khoảng $log N$) và bậc của các đỉnh (số cạnh nối với đỉnh đó) sẽ thấp (cũng chỉ là $log N$)).
    Ngoài ra, cây được sinh ra sẽ không hoàn toàn ngẫu nhiên. Những đỉnh có chỉ số thấp hơn thường sẽ có bậc cao hơn (đỉnh thứ $i$ sẽ có bậc kì vọng là $1 + \sum^{n}_{j=i+1} \frac{1}{j - 1}$), và cây sẽ có xu hướng gần với đường thẳng hơn.
- Sinh bằng Prüfer code:
    - Đầu tiên ta sinh ra $N$ -- số đỉnh của cây.
    - Sau đó, ta sinh $N - 2$ số $A_1, A_2,...,A_{N-2}$ trong khoảng $[1, N]$ - các số không nhất thiết phải phân biệt. Dựa vào dãy này, ta tạo ra cây bằng cách như sau:
    - Khi bắt đầu, chúng ta có $N$ đỉnh và chưa có cạnh nào
    - Ta định nghĩa ```deg[i]``` với $1 \le i \le N$ là số lần xuất hiện của $i$ trong dãy $A_1, A_2,..., A_{N-2}$.
    - Ở bước thứ $i (1 \le i \le N - 2)$, ta tìm vị trí $u$ nhỏ nhất có ```deg[u] = 0```, nối đỉnh $u$ với đỉnh $A_i$, và trừ ```deg[u]``` và ```deg[a[i]]``` đi $1$ (ta trừ ```deg[u]``` để đảm bảo rằng không bao giờ chọn $u$ nữa).
...
- Đỉnh $i$ nối với $i+1$ với mọi $1 \le i \le N - 1$ (đường thẳng).
- Đỉnh $1$ nối với $N - 1$ đỉnh còn lại (ngôi sao).
- Đỉnh $\lfloor{\frac{x}{2}}\rfloor$ nối với đỉnh $x$ với $2 \le x \le N$ (cây nhị phân hoàn chỉnh).
...
- Các điểm nằm ở trục toạ độ, $3$ điểm thẳng hàng (có thể rơi vào trường hợp chia cho ).
